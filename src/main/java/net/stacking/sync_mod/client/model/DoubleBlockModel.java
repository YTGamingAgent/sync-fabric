...
public abstract class DoubleBlockModel extends Model {
    ...
    protected ModelPart createCuboid(int textureOffsetU, int textureOffsetV, float x, float y, float z, float boxOffsetX, float boxOffsetY, float boxOffsetZ, float xSize, float ySize, float zSize) {
        ModelPart.Cuboid cuboid = new ModelPart.Cuboid(
                textureOffsetU, textureOffsetV,
                boxOffsetX, boxOffsetY, boxOffsetZ,
                xSize, ySize, zSize,
                0, 0, 0,
                false,
                this.textureWidth, this.textureHeight,
    ...
    /**
     * Convenience overload for call sites that don't pass a tint color.
     */
    public void render(DoubleBlockProperties.Type type, MatrixStack matrices, VertexConsumer vertices, int light, int overlay) {
        this.render(type, matrices, vertices, light, overlay, 0xFFFFFF);
    }
