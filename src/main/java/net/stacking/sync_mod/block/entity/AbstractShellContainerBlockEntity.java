...
import net.stacking.sync_mod.api.networking.ShellDestroyedPacket;
...
import net.stacking.sync_mod.util.ItemUtil;
...
public abstract class AbstractShellContainerBlockEntity extends BlockEntity implements ShellStateContainer, DoubleBlockEntity, TickableBlockEntity, Inventory {
    ...
    protected final BooleanAnimator sprayerAnimator;
    ...
    public AbstractShellContainerBlockEntity(BlockEntityType<?> type, BlockPos pos, BlockState state) {
        ...
        this.sprayerAnimator = new BooleanAnimator(false, 0.05F);
    }
    ...
    protected Optional<AbstractShellContainerBlockEntity> getBottomPart() {
        if (this.bottomPart == null && this.world != null) {
            this.bottomPart = AbstractShellContainerBlock.isBottom(this.getCachedState())
                    ? this
                    : (this.world.getBlockEntity(this.pos.offset(Direction.DOWN)) instanceof AbstractShellContainerBlockEntity x ? x : null);
        }
    ...
    private void updateComparatorOutput(World world, BlockPos pos, BlockState state) {
        ...
        if (this.progressComparatorOutput != currentProgressOutput) {
            ...
            if (state.get(AbstractShellContainerBlock.OUTPUT) == ComparatorOutputType.PROGRESS) {
                world.updateComparators(pos, state.getBlock());
            }
            if (topPartState.contains(AbstractShellContainerBlock.OUTPUT) &&
                    topPartState.get(AbstractShellContainerBlock.OUTPUT) == ComparatorOutputType.PROGRESS) {
        ...
        if (this.inventoryComparatorOutput != currentInventoryOutput) {
            ...
            if (state.get(AbstractShellContainerBlock.OUTPUT) == ComparatorOutputType.INVENTORY) {
                world.updateComparators(pos, state.getBlock());
            }
            if (topPartState.contains(AbstractShellContainerBlock.OUTPUT) &&
                    topPartState.get(AbstractShellContainerBlock.OUTPUT) == ComparatorOutputType.INVENTORY) {
                world.updateComparators(topPartPos, topPartState.getBlock());
            }
        }
    }
    ...
    public void onClientTick(World world, BlockPos pos, BlockState state) {
        ...
        boolean sprayersActive = this.shell != null
                && this.shell.getProgress() >= ShellState.PROGRESS_PRINTING
                && this.shell.getProgress() < ShellState.PROGRESS_DONE;
        this.sprayerAnimator.setValue(sprayersActive);
        this.sprayerAnimator.step();
    }
    ...
    @Environment(EnvType.CLIENT)
    public float getSprayerProgress(float tickDelta) {
        return this.getBottomPart().map(x -> x.sprayerAnimator.getProgress(tickDelta)).orElse(0f);
    }

    @Override
    public DoubleBlockProperties.Type getBlockType(BlockState state) {
        // Map DoubleBlockHalf to DoubleBlockProperties.Type
        net.minecraft.block.enums.DoubleBlockHalf half = AbstractShellContainerBlock.getShellContainerHalf(state);
        return half == net.minecraft.block.enums.DoubleBlockHalf.LOWER
    ...
    protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
        ...
        if (this.shell != null) {
            nbt.put("shell", this.shell.writeNbt(new NbtCompound(), registryLookup));
        }
    ...
    public void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
        ...
        this.shell = nbt.contains("shell") ? ShellState.fromNbt(nbt.getCompound("shell"), registryLookup) : null;
    ...
    public boolean isValid(int slot, ItemStack stack) {
        ...
        if (isOffHandSlot) {
            return ItemUtil.getPreferredEquipmentSlot(stack) == EquipmentSlot.OFFHAND ||
                    inventory.main.stream().noneMatch(x -> x.isEmpty() ||
                            (x.getCount() + stack.getCount()) <= x.getMaxCount() && ItemStack.areItemsAndComponentsEqual(x, stack));
        }
    ...
    public ItemStack getStack(int slot) {
        return this.getBottomPart()
                ...
                .map(x -> x.shell.getInventory().getStack(reorderSlotIndex(slot, x.shell.getInventory())))
    ...
    public int size() {
        return this.getBottomPart()
                .map(x -> x.shell == null || x.shell.getProgress() < ShellState.PROGRESS_DONE ? 0 : x.shell.getInventory().size())
                .orElse(0);
    }
    ...
    public boolean isEmpty() {
        return this.getBottomPart()
                .map(x -> x.shell == null || x.shell.getInventory().isEmpty())
                .orElse(true);
    }
