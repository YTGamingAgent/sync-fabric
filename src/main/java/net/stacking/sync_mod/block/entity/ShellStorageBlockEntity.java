...
public class ShellStorageBlockEntity extends AbstractShellContainerBlockEntity
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        // Connector / shell insert-eject arms
        controllers.add(new AnimationController<>(this, "connector", 0, state -> {
            float progress = this.getConnectorProgress(0);
            if (progress > 0.5f) {
                return state.setAndContinue(SHELL_INSERT);
            } else if (progress > 0f) {
                return state.setAndContinue(SHELL_EJECT);
            }
    ...
    @Override
    public long extract(long amount, TransactionContext context) { return 0; }
